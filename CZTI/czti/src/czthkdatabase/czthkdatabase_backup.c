...
int main(int argc,char*argv[])
{
	...
	char qua[10];
	start_time=clock();
	fitsfile *infptr, *tctin, *orbin, *lbtin,*evtin;
	...
	int second_count,command,cpm_count;
	//long startindex=0;
	long orbrows,evtrows;
	float *Latitude,*Longitude,*Altitude,**TEMP,temperature;
	double *orbTime, *lbtTime,*evtTime;
	...
	if (mysql_real_connect(con, "192.168.11.35", "root", "C@dm1um", "czti_hk", 0, NULL, 0) == NULL) 
  	{
	...
	evtrows=getNumrows(evt_infile,8);

	printf("\n #####evt no of rows: %ld",evtrows);
	...
	TEMP=(float**)malloc(sizeof(float*)*evtrows);
	for(z=0;z<evtrows;z++){
	TEMP[z]=(float*)malloc(sizeof(float)*16);
	}

	//dynamic allocating lbt data
	lbtTime=(double*)malloc(sizeof(double*)*lbtrows);
	CPM_Rate=(int*)malloc(sizeof(int*)*lbtrows);
	evtTime=(double*)malloc(sizeof(double*)*evtrows);
	QUAD_FLAG=(int*)malloc(sizeof(int*)*evtrows);
	...
	//opening evt file 
	fits_open_file(&evtin,evt_infile,READONLY,&status);
	...
	//reading evt file
	fits_movabs_hdu(evtin,8,NULL,&status);
	if(status)
	{
		printf("Error in moving HDU\n");        
		fits_report_error(stderr,status);
	}
	fits_read_col(evtin,TDOUBLE,1,1,1,evtrows,NULL,evtTime,NULL,&status);
	for(z=0;z<evtrows;z++){
	fits_read_col(evtin,TFLOAT,3,z+1,1,16,NULL,TEMP[z],NULL,&status);
	}
	//for(z=0;z<10;z++){
	//printf("\n####TEMP[0]:%f",TEMP[z][15]);
	//printf("\n####TEMP[1]:%f",TEMP[0][1]);
	//}
	fits_read_col(evtin,TINT,1,1,1evtrows,NULL,QUAD_FLAG,NULL,&status);

	for(curqod=0;curqod<4;curqod++)
	{
	
		//sprintf(qua,"Quadrant%d",curqod);
		//fs=fopen(qua, "w");
		fits_movabs_hdu(infptr,curqod+2,NULL,&status);
		if(status)
		{
			printf("Error in moving hdu\n");
			fits_report_error(stderr,status);
		...
		do
		{
			...
			if(pktid==0)
			{
                ...
                command=(unsigned int)(((data[8]<<8)) | (data[9] & 0xffff));
...
*/
	...
	*altitude=getlagvalue(4,nearer,nearer_altvalue,outtime);
	/*if(orbTime[index]<=184786161.027121 && orbTime[index]>=184786160.027121){
		printf("\n orbtime:%lf",orbTime[index]);
		for(i=0;i<4;i++){
			printf("\nlatitude:%f\tlongitude:%f\taltitude:%f\n",Latitude[index],Longitude[index],Altitude[index]);
			printf("\ninterpolated latitude:%f\tinterpolated longitude:%f\tinterpolated altitude:%f",*latitude,*longitude,*altitude);
		}
	}*/
	/*printf("\nTime:%lf",outtime);
	printf("\nfound time:%lf",orbTime[index]);
	printf("\ninterpolated latitude:%f\tinterpolated longitude:%f\tinterpolated altitude:%f",*latitude,*longitude,*altitude);
	*/
}	
double getlagvalue(long n, double time_arr[],float value_arr[],double sectime)
...
double searchgetlag_evt(double outtime,double orbTime[],float TEMP[],long evtrows,float *temperature)
{
	...
	nearer_value[0]=TEMP[index];
	nearer_value[1]=TEMP[index+1];
	nearer_value[2]=TEMP[index+2];
	nearer_value[3]=TEMP[index+3];
	...
	nearer_value[0]=TEMP[index-2];
	nearer_value[1]=TEMP[index-1];
	nearer_value[2]=TEMP[index];
	nearer_value[3]=TEMP[index+1];
...
*/
	*temperature=getlagvalue(4,nearer,nearer_latvalue,outtime);
}	
double getlagvalue(long n, double time_arr[],float value_arr[],double sectime)
{	
	long b,length;
