...
int cztimage::cztimageProcess(){
    ...
    /*intializing mask array to contain zeros*/
   ...
   int prev_peaks=nsources,prev_nbkgd=nBkgdCoef;
    ...
    if(nBkgdCoef>0){
        for(int i=1;i<nBkgdCoef;i++)
            for(int j=0;j<XSIZE*YSIZE;j++)
                bkgdshadows[0][j]=bkgdshadows[0][j]+bkgdshadows[i][j];           //this will give total component of background
    ...
    //constructing background subtracted images
    oversample(); //to oversample dpi and mask
...
int cztimage::getMaskArray(){
    ...
    if(status!=0) 
    { LOG(ERROR)<<"***Error opening mask file***"; fits_report_error(stderr,status);  return (EXIT_FAILURE);; }
    ...
    for(int i=0;i<NUMQUAD;i++){
        ...
        if(status!=0) 
        { LOG(ERROR)<<"***Error moving HDU***"; fits_report_error(stderr,status);  return (EXIT_FAILURE);; }
        ...
        if(hdutype==IMAGE_HDU && qid==i){
            ...
            if(status!=0)
            { LOG(ERROR)<<"***Error reading mask pixels***";  fits_report_error(stderr,status);  return (EXIT_FAILURE);;}
    ...
    fits_close_file(fptr,&status);
    if(status!=0)
    { LOG(ERROR)<<"***Error closing maskfile "<<maskfile64x64<<" ***"; fits_report_error(stderr,status);  return (EXIT_FAILURE);; }
    return (EXIT_SUCCESS);
 }
...
int cztimage::getDPI(){
    ...
    if(status!=0) 
    { LOG(ERROR)<<"***Error opening DPI file***"; fits_report_error(stderr,status);  return (EXIT_FAILURE);; }
    ...
    for(int i=0;i<NUMQUAD;i++){
        ...
        if(qid==i){
            ...
            if(status!=0)
            { LOG(ERROR)<<"***Error reading DPI pixels***";  fits_report_error(stderr,status);  return (EXIT_FAILURE);;}
            r=(qid/NUMQUAD_ROW)*(YSIZE/NUMQUAD_COL);
            c=(qid%NUMQUAD_ROW)*(XSIZE/NUMQUAD_ROW);
            index=0;
            for(int u=r;u<r+(YSIZE/NUMQUAD_COL);u++)
    ...
    fits_close_file(fptr,&status);
    if(status)
    { LOG(ERROR)<<"***Error closing dpifile "<<dpifile<<" ***"; fits_report_error(stderr,status);  return (EXIT_FAILURE);; }
    return(EXIT_SUCCESS);
}
...
//int removeKnownSources(char *knownSourcesShadowfile,float *dpi,int rows,int cols){
//    fitsfile *fptr;
//    int status=0;
//    fits_open_file(&fptr,knownSourcesShadowfile,READONLY,&status);
//    if(status) { fits_report_error(stderr,status); return (EXIT_FAILURE); }
//    int nhdu=0;
...
//        return (EXIT_SUCCESS);
//    }
//    float *shadow=new float[rows*cols];
//    float *finalshadow=new float[rows*cols];    //final shadow is combination of all known sources
//    long fpixel[2];  fpixel[0]=fpixel[1]=1;
//    for(int i=2;i<=nhdu;i++){
//        fits_movrel_hdu(fptr,1,NULL,&status);
//        if(status) { fits_report_error(stderr,status); return (EXIT_FAILURE); }
//        fits_read_pix(fptr,TFLOAT,fpixel,rows*cols,NULL,shadow,NULL,&status);
//        if(status) { fits_report_error(stderr,status); return (EXIT_FAILURE); }
//        finalshadow[i]=finalshadow[i]+shadow[i];
//    }
//    for(int i=0;i<rows*cols;i++)
//        dpi[i]=dpi[i]-finalshadow[i];
//    delete[] finalshadow,shadow;
//    fits_close_file(fptr,&status);
//    if(status) { fits_report_error(stderr,status); return (EXIT_FAILURE); }
//    LOG(INFO)<<"Known Sources removed from shadow";
//    return (EXIT_SUCCESS);
//}
...
int cztimage::writeSkyImage(){
    ...
    int naxis=2;
    long naxes[2];  naxes[0]=skyimgsize;   naxes[1]=skyimgsize;
    long fpixel[2];  fpixel[0]=fpixel[1]=1;
...
int cztimage::identifyPeaks(){
    ...
    int r,c; 
    int rowcen=skyimgsize/2;    int colcen=skyimgsize/2;
   ...
   frow.clear(); fcol.clear(); intensity.clear();
   return (EXIT_SUCCESS);
}
