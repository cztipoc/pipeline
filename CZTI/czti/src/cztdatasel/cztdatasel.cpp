    ...
    if(strcasecmp(gtitype,"COMMON")==0)
    {
        ...
        for(i=0;i<5;i++)
        {
			...
			if(i<4)
		        ...
		        sprintf(extname,"VETOSPECTRUM");
			LOG(INFO)<<"Processing "<<extname;
...
//Compute exposure time given GTI extension fitsfile pointer
int computeExptime(fitsfile *fgti,double *exp_time, double *tstart, double *tstop)
{
    ...
    double gti_start[nrows],gti_stop[nrows],exposure_time=0.;
