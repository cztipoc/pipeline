...
//		LOG(INFO)<<"TX TY "<<att_thx[i]<<"  "<<att_thy[i];
	...
	*/	
	...
	for (qseq=0;qseq<quadVec.size();qseq++)
	{
		...
		spec.clear();
		lc.clear();
        lcstartT=(long)head_tstart;
        lcstopT=(long)head_tstop+1;

        long nlcbins=ceil((double)(lcstopT-lcstartT)/(double)binsize);
		...
		for (ievt=0;ievt<evtnrows;ievt++)
		{
			...
			if((evtPI[ievt]>LLD[detpix_index])||(applyLLD==0))
			{
				...
				if((long)((evttime[ievt]-lcstartT)/binsize)<nlcbins)
				{	
        ...
        if(outtypeflag==LC||outtypeflag==SPECLC)
        {
			...
			double frac_binsize=(lv_timearray[1]-lv_timearray[0]);
	
			float binrem=fmod((float)binsize,(float)frac_binsize);
			long binratio=(long)(binsize/frac_binsize);

			if(binrem==0)
			{
				long fr_startbin=(long)(((double)lcstartT-lv_timearray[0]-frac_binsize/2.0)/binsize)+1;
